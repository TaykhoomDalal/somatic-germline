...
def plotclinicalactionability(annotatedclinicalfile, outfile, parameters):
    ...
    if ncat > 0:
        ...
        order = reversed(sorted(list(range(ncat)),key=lambda x:(catactionabilityarray[x],catoncogenicarray[x])))
        ...
        for level in extlevels:
            ...
            accumlevelcancerperc = list(map(sum, list(zip(accumlevelcancerperc,levelcancerperc))))
